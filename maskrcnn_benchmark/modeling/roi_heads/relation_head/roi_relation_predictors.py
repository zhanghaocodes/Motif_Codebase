...
@registry.ROI_RELATION_PREDICTOR.register("MotifsLikePredictor_Resample")
class MotifsLikePredictor_Resample(nn.Module):
    def __init__(self, config, in_channels):
        super(MotifsLikePredictor_Resample, self).__init__()
        self.cls_cur_num = self.init_cur_num()
        self.cls_tar_num = [807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807, 779, 702, 679, 652, 641, 580, 512, 511, 493, 485, 435, 369, 343, 327, 289, 265, 263, 224, 198, 172, 153, 134, 128, 49, 5]
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        for i in range(len(rel_labels)):
            ...
            for j in range(rel_label.size(0)):
                if self.cls_cur_num [rel_label[j]] <= self.cls_tar_num[rel_label[j]]:
                    self.cls_cur_num[rel_label[j]] +=1
                    if i==0:
                        tmp_union_features.append(union_features[j])
                    else:
                        tmplength=0
                        for m in range(i):
                            tmplength+=rel_labels[m].size(0)
                        tmp_union_features.append(union_features[j+tmplength])
                    tmpRel.append(rel_label[j])
                    tmpRel_pair.append(rel_pair_idxs[i][j])
            tmpRel = torch.tensor(tmpRel).cuda()
            tmpList_rel_labels.append(tmpRel)
            if not tmpRel_pair:
                self.resampleFlag= True
                self.cls_cur_num = self.init_cur_num()
                for j in range(rel_label.size(0)):
                    if self.cls_cur_num[rel_label[j]] <= self.cls_tar_num[rel_label[j]]:
                        self.cls_cur_num[rel_label[j]] += 1
                        ...
                        tmpRel.append(rel_label[j])
                        tmpRel_pair.append(rel_pair_idxs[i][j])
            tmpRel_pair = torch.stack(tmpRel_pair).cuda()
            tmpList_rel_pairs.append(tmpRel_pair)


        rel_pair_idxs = tmpList_rel_pairs
        rel_labels = tmpList_rel_labels
        union_features = torch.stack(tmp_union_features).cuda()
        ...
        for pair_idx, head_rep, tail_rep, obj_pred in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds):
            prod_reps.append( torch.cat((head_rep[pair_idx[:,0]], tail_rep[pair_idx[:,1]]), dim=-1) )
            a = obj_pred[pair_idx[:,0]]
            b = obj_pred[pair_idx[:,1]]
            pair_preds.append( torch.stack((obj_pred[pair_idx[:,0]], obj_pred[pair_idx[:,1]]), dim=1) )
        prod_rep = cat(prod_reps, dim=0)
        pair_pred = cat(pair_preds, dim=0)
